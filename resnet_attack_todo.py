...
class ResnetPGDAttacker:
    ...
    def pgd_attack(self, image, label, eps=None, alpha=None, steps=None):
        ...
        # pass  # TODO
        adv_images = images + torch.randn_like(images).uniform_(-eps, eps)
        adv_images = torch.clamp(adv_images, 0, 1)  # Ensure values remain valid after initialization
        ...
        for _ in range(steps):
            ...
            # Projection step
            # pass  # TODO
            delta = torch.clamp(adv_images - images, min=-eps, max=eps)  # Limit the perturbation to the epsilon ball
            adv_images = torch.clamp(images + delta, 0, 1)  # Keep the perturbed image in the valid pixel range
            adv_images = adv_images.detach()
